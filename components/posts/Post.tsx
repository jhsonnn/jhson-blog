...
//   isRandomPosts = false,
// }) => {
//   //category가 none이거나 tags가 없는 경우 렌더링 제외
...
//     <Link href={`/${category}/${slug}`} passHref>
//       <div
//         className={`relative min-w-full min-h-full max-w-xl mx-auto bg-neutral-100 dark:bg-neutral-700 rounded-3xl shadow-lg overflow-hidden transition-transform duration-300 hover:scale-[1.01] hover:shadow-xl mb-6 cursor-pointer ${
//           isRandomPosts ? 'h-[280px]' : ''
//         }`}
//       >
//         <div
...
//   isRandomPosts = false,
// }) => {
//   // category가 "none"이거나 tags가 없는 경우 렌더링 제외
//   if (category === 'none' || tags.length === 0) return null;

//   // ✅ 날짜 변환 (ISO 포맷 → YYYY년 MM월 DD일)
//   let formattedDate = 'Unknown Date';
//   if (date) {
//     try {
//       formattedDate = new Date(date).toISOString().split('T')[0]; // YYYY-MM-DD 변환
//     } catch (error) {
